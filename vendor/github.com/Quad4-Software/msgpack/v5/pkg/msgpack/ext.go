...
// extTypes maps a registered ext id to its decode info. It is a sync.Map
// because decodeInterfaceExt reads it on every ext decode while
// RegisterExtDecoder and unregisterExtDecoder may mutate it from another
// goroutine; a plain map would race under concurrent use.
var extTypes sync.Map // int8 -> *extInfo
...
func (d *Decoder) skipExt(c byte) error {
	...
	// Skip the type byte and the payload in two calls. A single
	// skipN(n + 1) overflows int when n is math.MaxInt32 on 32-bit
	// platforms, turning a forged ext32 header into a negative slice
	// bound panic inside the grow-bounded reader.
